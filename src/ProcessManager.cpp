...
void ProcessManager::run() {
    ...
    int processCount = config.getProcesses().size(),
        stockCount = config.getStocks().size();
    if (processCount > 10 || stockCount > 5) generations = 150;
    if (processCount > 15 || stockCount > 10) generations = 200;
...
void ProcessManager::generateOutput(const Individual &bestSolution) {
    ...
    for (const string &stockName : relevantStocks) {
        ...
        if (stockIt != result.finalStocks.end()) quantity = stockIt->second;
